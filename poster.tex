...
\begin{column}{0.46\paperwidth}
  ...
  \begin{block}{Introduction}
    ...
    \begin{itemize}
        ...
        \item TabPFNv2 → nanoTabPFN → modded-nanoTabPFN
      \end{itemize}
  ...
  \begin{block}{Improvement techniques}
    ...
    \begin{itemize}
      ...
      \item Scaled Dot-Product Attention rewrite with explicit QKV
      \item Pre-norm transformer blocks \cite{geiping2022crammingtraininglanguagemodel}
      \item Compile TransformerEncoderLayer forward
      \item bfloat16 autocast in training and inference
      \item Set float32 matmul precision to high
    ...
    \begin{itemize}
      \item GoLU activation function \cite{das2025gompertz}
      \item RoRa \cite{roratab2024}
    \end{itemize}

    \heading{The following techniques were evaluated but did not lead to improvements:}
    \begin{itemize}
      \item Initialise linear layers with Xavier initialization \cite{pmlr-v9-glorot10a}
      \item Disable all linear layer biases \cite{geiping2022crammingtraininglanguagemodel}
    \end{itemize}
...
\begin{column}{0.44\paperwidth}
  ...
  \begin{block}{Record history}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=0.95\linewidth]{img/260211-114840-records-plot-x-time-y-val.png}
      \label{fig:record-history-time}
    \end{figure}

    \begin{figure}
      \centering
      \includegraphics[width=0.95\linewidth]{img/260211-114947-records-plot-x-datasets-y-val.png}
    ...
    \begin{center}
      ...
      \begin{tabular}{@{}l p{0.46\linewidth} p{0.31\linewidth}@{}}
        \toprule
